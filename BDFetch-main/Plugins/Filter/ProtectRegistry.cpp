...
static PWCHAR GetFullPath(PVOID RootObject, PUNICODE_STRING CompleteName, PULONG OutLength) {
    ...
    Buffer = (PWCHAR)BDFetchAllocate(TotalSize);
    if (!Buffer) {
            ...
            if (KeyName) {
                if (KeyName->Buffer) {
                    if (!IsBamRegistryPath(KeyName)) {
                        ...
                        if (FullSize <= 0xFFFF) {
                            PWCHAR Buffer = (PWCHAR)BDFetchAllocate(FullSize);
                            if (Buffer) {
                                UNICODE_STRING FullPath;
...
NTSTATUS InitializeRegistryProtection(PDRIVER_OBJECT DriverObject) {
    static UNICODE_STRING Altitude = RTL_CONSTANT_STRING(L"320000.BDFetch.Cm");
    NTSTATUS status = CmRegisterCallbackEx(RegistryCallback, &Altitude, DriverObject, NULL, &Cookie, NULL);

    if (!NT_SUCCESS(status)) {
        static UNICODE_STRING FallbackAltitude = RTL_CONSTANT_STRING(L"320000.BDFetch.Cm.Fallback");
