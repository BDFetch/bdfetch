...
static VOID FreeList(PRULE_NODE* Head) {
    ...
    while (Current) {
        ...
        if (Current->Pattern.Buffer) BDFetchFree(Current->Pattern.Buffer);
        BDFetchFree(Current);
        Current = Next;
    }
    *Head = NULL;
...
static VOID AddRule(PRULE_NODE* Head, PUNICODE_STRING RuleStr) {
    ...
    PRULE_NODE Node = (PRULE_NODE)BDFetchAllocate(sizeof(RULE_NODE));
    if (!Node) return;

    SIZE_T Size = RuleStr->Length + sizeof(WCHAR);
    Node->Pattern.Buffer = (PWCHAR)BDFetchAllocate(Size);
                        ...
                        if (*Ptr == '"') {
                            ...
                            if (Ptr < End && *Ptr == '"') {
                                ...
                                if (UTF8Len > 0) {
                                    ...
                                    if (WideSize > 0) {
                                        PWCHAR WideBuffer = (PWCHAR)BDFetchAllocate(WideSize + sizeof(WCHAR));
                                        if (WideBuffer) {
                                            ULONG ResultSize = 0;
...
static VOID RemoveRule(PRULE_NODE* Head, PUNICODE_STRING RuleStr) {
    ...
    while (Current) {
        if (RtlEqualUnicodeString(&Current->Pattern, RuleStr, TRUE)) {
            ...
            if (ToDelete->Pattern.Buffer) BDFetchFree(ToDelete->Pattern.Buffer);
            BDFetchFree(ToDelete);
        }
        else {
            Previous = Current;
            Current = Current->Next;
...
NTSTATUS LoadRulesFromDisk(PUNICODE_STRING RegistryPath) {
    ...
    Info = (PKEY_VALUE_PARTIAL_INFORMATION)BDFetchAllocate(ResultLength);
    if (!Info) {
        ZwClose(RegHandle);
    ...
    if (Info->Type == REG_EXPAND_SZ || Info->Type == REG_SZ) {
        ...
        PathBuffer = (PWCHAR)BDFetchAllocate(PathBufferSize);
    ...
    if (!FinalPath.Buffer) {
        if (PathBuffer) BDFetchFree(PathBuffer);
    ...
    if (NT_SUCCESS(status)) {
        ...
        if (NT_SUCCESS(status) && FileInfo.EndOfFile.LowPart > 0) {
            PVOID FileBuffer = BDFetchAllocate(FileInfo.EndOfFile.LowPart + 1);
            if (FileBuffer) {
...
BOOLEAN IsProcessTrusted(HANDLE ProcessId) {
    if ((ULONG)(ULONG_PTR)ProcessId == GlobalData.BDFetchPid) return TRUE;
...
static BOOLEAN IsHighEntropy(PVOID Buffer, ULONG Length) {
    ...
    PUSHORT Histogram = (PUSHORT)BDFetchAllocate(256 * sizeof(USHORT));
...
NTSTATUS SendMessageToUser(ULONG Code, ULONG Pid, PWCHAR Path, USHORT PathSize) {
    ...
    if (GlobalData.ClientPort) {
        PBDFetch_MESSAGE msg = (PBDFetch_MESSAGE)BDFetchAllocate(sizeof(BDFetch_MESSAGE));
        if (msg) {
            ...
            status = FltSendMessage(GlobalData.FilterHandle, &GlobalData.ClientPort, msg, sizeof(BDFetch_MESSAGE), NULL, NULL, &timeout);
