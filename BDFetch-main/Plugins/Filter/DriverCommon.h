...
constexpr auto BDFetch_PORT_NAME = L"\\BDFetch_Output_Pipe";
constexpr auto MAX_PATH_LEN = 1024;
constexpr auto BDFetch_POOL_TAG = 'SAYP';
...
typedef struct _BDFetch_MESSAGE {
    ULONG MessageCode;
    ULONG ProcessId;
    WCHAR Path[MAX_PATH_LEN];
} BDFetch_MESSAGE, * PBDFetch_MESSAGE;

typedef struct _BDFetch_USER_MESSAGE {
    ULONG Command;
    WCHAR Path[MAX_PATH_LEN];
} BDFetch_USER_MESSAGE, * PBDFetch_USER_MESSAGE;
...
FORCEINLINE PVOID BDFetchAllocate(SIZE_T Size) {
    return ExAllocatePool2(POOL_FLAG_NON_PAGED, Size, BDFetch_POOL_TAG);
}

FORCEINLINE VOID BDFetchFree(PVOID Ptr) {
    if (Ptr) ExFreePoolWithTag(Ptr, BDFetch_POOL_TAG);
}