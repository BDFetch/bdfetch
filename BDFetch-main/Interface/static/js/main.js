...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const triggerScan = (method) => {
        ...
        appState.totalFilesToScan = 0;
        appState.scannedFilesCount = 0;
        appState.scanProgressPercent = 0;
        appState.currentScannedPath = '';
        appState.virusMap.clear();
    ...
    window.updateScanProgress = (path) => {
        ...
        appState.currentScannedPath = path;
        
        // 累加已扫描文件计数
        if (appState.totalFilesToScan > 0 && appState.scannedFilesCount < appState.totalFilesToScan) {
            appState.scannedFilesCount++;
            let percent = Math.min(95, Math.floor((appState.scannedFilesCount / appState.totalFilesToScan) * 100));
            if (percent >= appState.scanProgressPercent) {
                appState.scanProgressPercent = percent;
            }
        }
        
        if (title) { 
            title.setAttribute('data-i18n', 'status_scanning'); 
            title.textContent = getMsg('status_scanning'); 
        }
        
        if (text) {
            if (appState.totalFilesToScan > 0) {
                const fileName = path ? (path.split('\\').pop() || path.split('/').pop() || '扫描中...') : '扫描中...';
                const displayName = fileName.length > 40 ? fileName.substring(0, 37) + '...' : fileName;
                text.innerHTML = `${displayName}<br><span style="font-size: 12px; color: var(--accent-color);">扫描进度: ${appState.scanProgressPercent}% (${appState.scannedFilesCount}/${appState.totalFilesToScan})</span>`;
            } else {
                if (path) {
                    ...
                    text.removeAttribute('data-dynamic-msg');
                    text.innerHTML = `${displayName}<br><span style="font-size: 12px; color: var(--accent-color);">正在扫描...</span>`;
                } else {
                    text.innerHTML = `扫描中...<br><span style="font-size: 12px; color: var(--accent-color);">正在初始化...</span>`;
                }
            }
        }
    };
    ...
    window.updateDeleteProgress = (path) => {
        ...
        if (text) {
            const fileName = path ? (path.split('\\').pop() || path.split('/').pop() || path) : '处理中...';
            const displayName = fileName.length > 50 ? fileName.substring(0, 47) + '...' : fileName;
            text.removeAttribute('data-i18n');
            text.removeAttribute('data-dynamic-msg');
            text.innerHTML = `${displayName}<br><span style="font-size: 12px; color: var(--accent-color);">正在删除...</span>`;
        }
    };

    // 设置总文件数
    window.setTotalScanFiles = (total) => {
        if (total > 0) {
            appState.totalFilesToScan = total;
            appState.scannedFilesCount = 0;
            appState.scanProgressPercent = 0;
        }
    };
    ...
    window.finishScan = (msgData, count) => {
        ...
        appState.scanProgressPercent = 100;
        ...
        if (text) {
            ...
            if (typeof msgData === 'object' && msgData !== null) {
                msg = msgData[appState.lang] || msgData["english_switch"];
                text.dataset.dynamicMsg = JSON.stringify(msgData);
            } else {
                msg = getMsg(msgData);
            }
            text.removeAttribute('data-i18n');
            text.innerHTML = `${msg}<br><span style="font-size: 12px; color: var(--accent-color);">扫描完成: 发现 ${count} 个威胁 | 共扫描 ${appState.scannedFilesCount} 个文件</span>`;
        }
    ...
    window.triggerContextScan = (targets) => {
        ...
        if (window.pywebview) {
            ...
            if (text) { 
                text.removeAttribute('data-i18n');
                text.removeAttribute('data-dynamic-msg');
                text.innerHTML = `正在启动扫描...<br><span style="font-size: 12px; color: var(--accent-color);">准备就绪...</span>`;
            }
    ...
    const bindToolButtons = () => {
        document.querySelectorAll('.tool-btn').forEach(btn => {
            btn.removeEventListener('click', btn._toolHandler);
            btn._toolHandler = (e) => {
                e.stopPropagation();
                const url = btn.getAttribute('data-url');
                if (url && window.pywebview) {
                    window.pywebview.api.open_url(url);
                }
            };
            btn.addEventListener('click', btn._toolHandler);
        });
    };
