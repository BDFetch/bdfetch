...
#define AppName "BDFetch"
#define AppVersion "1.0.0.0"
#define AppPublisher "BDFetch Security"
#define AppURL "https://bdfetch.66ghz.com/sd/"
#define AppExeName "BDFetch.exe"
...
OutputDir=Output
OutputBaseFilename=BDFetch_Setup
UsePreviousTasks=no
...
Source: "Redist\VC_redist.x64.exe"; DestDir: "{tmp}\BDFetch_Redist"; Flags: deleteafterinstall ignoreversion
Source: "Redist\MicrosoftEdgeWebview2Setup.exe"; DestDir: "{tmp}\BDFetch_Redist"; Flags: deleteafterinstall ignoreversion

[Registry]
Root: HKCU; Subkey: "Software\Classes\*\shell\BDFetch_Scan"; Flags: uninsdeletekey dontcreatekey
Root: HKCU; Subkey: "Software\Classes\Directory\shell\BDFetch_Scan"; Flags: uninsdeletekey dontcreatekey

[UninstallDelete]
Type: filesandordirs; Name: "{commonappdata}\BDFetch"
...
[Run]
Filename: "{tmp}\BDFetch_Redist\VC_redist.x64.exe"; Parameters: "/quiet /norestart"; Flags: waituntilterminated runhidden; StatusMsg: "{cm:InstallingVCRuntime}"; Tasks: install_vcredist
Filename: "{tmp}\BDFetch_Redist\MicrosoftEdgeWebview2Setup.exe"; Parameters: "/silent /install"; Flags: waituntilterminated runhidden; StatusMsg: "{cm:InstallingWebView2Runtime}"; Tasks: install_webview2
...
english.AutoStartTask=Run BDFetch automatically at system startup
chinesesimplified.AutoStartTask=开机时自动运行 BDFetch
chinesetraditional.AutoStartTask=開機時自動執行 BDFetch
...
english.LegacyVersionDetected=An older version of BDFetch was detected. Please uninstall it manually before installing.
chinesesimplified.LegacyVersionDetected=检测到存在旧版 BDFetch。请先手动卸载旧版后，再运行本安装程序。
chinesetraditional.LegacyVersionDetected=檢測到存在舊版 BDFetch。請先手動卸載舊版後，再執行本安裝程式。
...
begin
  ...
  PSCommand := '-NoProfile -NonInteractive -ExecutionPolicy Bypass -WindowStyle Hidden -Command ' +
               ...
               'Register-ScheduledTask -TaskName ''BDFetch_Security_ATS'' -Action $Action -Trigger $Trigger -Settings $Settings -RunLevel Highest -Force';
...
var
  ResultCode: Integer;
begin
  if not Exec(ExpandConstant('{sys}\schtasks.exe'), '/Delete /TN "BDFetch_Security_ATS" /F', '', SW_HIDE, ewWaitUntilTerminated, ResultCode) then
  begin
