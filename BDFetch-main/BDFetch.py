...
from BDFetch_Engine import sign_scanner, rule_scanner, pe_scanner, cloud_scanner
...
class BDFetch_MESSAGE(ctypes.Structure):
    _fields_ = [
...
class BDFetch_FULL_MESSAGE(ctypes.Structure):
    _fields_ = [
        ("Header", FILTER_MESSAGE_HEADER),
        ("Data", BDFetch_MESSAGE)
    ]

class BDFetch_USER_MESSAGE(ctypes.Structure):
    _fields_ = [
...
class WindowAPI:
    def __init__(self):
        ...
        if not self.check_singleton("BDFetch_Security_Mutex"):
            hwnd = self.user32.FindWindowW(None, "BDFetch Security")
            if hwnd:
                if "-quit" in self.args_BDFetch:
                    try:
                        cds = COPYDATASTRUCT()
                        cds.dwData = 3
                        cds.cbData = 0
                ...
                elif "-scan" in self.args_BDFetch:
                    try:
                        idx = self.args_BDFetch.index("-scan")
                        target = self.args_BDFetch[idx+1]
                        cds = COPYDATASTRUCT()
                        cds.dwData = 1
    ...
    def init_environ(self):
        ...
        if getattr(sys, 'frozen', False):
            self.file_BDFetch = self.norm_path(sys.executable)
        else:
            self.file_BDFetch = self.norm_path(os.path.abspath(sys.argv[0]))
            
        self.args_BDFetch = sys.argv[1:]
        self.path_BDFetch = os.path.dirname(self.file_BDFetch)
        self.pid_BDFetch = int(os.getpid())
        ...
        self.file_config = os.path.join(self.path_config, "BDFetch", "Config.json")
        self.file_log = os.path.join(self.path_config, "BDFetch", "Report.json")
        self.path_properties = os.path.join(self.path_BDFetch, "Engine", "Properties")
        self.path_pattern = os.path.join(self.path_BDFetch, "Engine", "Pattern")
        self.path_heuristic = os.path.join(self.path_BDFetch, "Engine", "Heuristic")
        self.path_protect = os.path.join(self.path_BDFetch, "Plugins", "Filter")
        self.path_drivers = os.path.join(self.path_protect, "BDFetch_Driver.sys")
    ...
    def _loc(self, text_dict):
        with self.lock_config:
            lang = self.BDFetch_config.get("language", "traditional_switch") if hasattr(self, 'BDFetch_config') else "traditional_switch"
    ...
    def load_config(self):
        with self.lock_config:
            ...
            if not os.path.exists(self.file_config):
                self.BDFetch_config = self.BDFetch_default.copy()
                ...
                try:
                    with open(self.file_config, "r", encoding="utf-8") as f:
                        data = json.load(f)
                        self.BDFetch_config = {**self.BDFetch_default, **data}
                    self.BDFetch_config["version"] = self.BDFetch_default["version"]
        ...
        with self.lock_config:
            try:
                ...
                with open(self.file_config, "w", encoding="utf-8") as f:
                    json.dump(self.BDFetch_config, f, indent=4, ensure_ascii=False)
    ...
    def update_config(self, key, value):
        with self.lock_update:
            with self.lock_config:
                ...
                old_value = self.BDFetch_config.get(key)
                if old_value == value:
                    return value
                self.BDFetch_config[key] = value
            ...
            if success:
                ...
                with self.lock_config:
                    self.BDFetch_config[key] = old_value
                if self._window:
    ...
    def get_config(self):
        with self.lock_config:
            return self.BDFetch_config.copy()
    ...
    def export_logs(self, log_ids=None):
        if self._window:
            path = self._window.create_file_dialog(webview.FileDialog.SAVE, directory='', save_filename='BDFetch_Logs.json')
            if path:
    ...
    def manage_named_list(self, list_key, files, action="add", lock_func=None):
        ...
        acted_items = []
        with self.lock_config:
            target_list = self.BDFetch_config.setdefault(list_key, [])
    ...
    def remove_list_items(self, list_key, paths_to_remove):
        with self.lock_config:
            target_list = self.BDFetch_config.get(list_key, [])
            ...
            self.BDFetch_config[list_key] = new_list
            
            if len(self.BDFetch_config[list_key]) < original_len:
...
####################################################################################################
    ...
    def get_app_icon(self):
        icon_path = os.path.join(self.path_BDFetch, "Interface", "static", "img", "icon.ico")
    ...
    def get_tray_text(self, key):
        texts = {
            "open_ui": {
                "traditional_switch": "開啟介面", "simplified_switch": "打开界面", "english_switch": "Open BDFetch",
                "japanese_switch": "BDFetch を開く", "korean_switch": "BDFetch 열기", "french_switch": "Ouvrir BDFetch",
                "spanish_switch": "Abrir BDFetch", "hindi_switch": "BDFetch खोलें", "arabic_switch": "فتح BDFetch",
                "russian_switch": "Открыть BDFetch", "slovenian_switch": "Odpri BDFetch"
            },
            "check_update": {
    ...
    def show_tray(self):
        ...
        self.tray_icon = pystray.Icon("BDFetch", self.get_app_icon(), "BDFetch Security", menu)
    ...
    def close(self, *args, **kwargs):
        ...
        with self.lock_config:
            self.BDFetch_config["process_switch"] = False
            self.BDFetch_config["document_switch"] = False
            self.BDFetch_config["system_switch"] = False
            self.BDFetch_config["driver_switch"] = False
            self.BDFetch_config["network_switch"] = False
    ...
    def register_context_menu(self, enable):
        paths = [r"Software\Classes\*\shell\BDFetch_Scan", r"Software\Classes\Directory\shell\BDFetch_Scan"]
        cmd_path = f'"{self.file_BDFetch}"' if getattr(sys, 'frozen', False) else f'"{self.python}" "{self.file_BDFetch}"'

        try:
            for path in paths:
                if enable:
                    self._reg_write(winreg.HKEY_CURRENT_USER, path, None, winreg.REG_SZ, "BDFetch Security Scan")
    ...
    def open_website(self):
        try:
            return webbrowser.open(self.BDFetch_config.get("api_host"))
        except Exception:
            return False
...
####################################################################################################
    ...
    def check_update(self):
        try:
            current = self.BDFetch_config.get("version", "0.0.0")
            j = requests.get("https://api.github.com/repos/87owo/BDFetch/releases/latest", headers={"Accept": "application/vnd.github+json", "User-Agent": "BDFetch"}, timeout=10).json()
            latest = str(j.get("tag_name") or j.get("name") or "").strip()
            page = j.get("html_url") or "https://github.com/87owo/BDFetch/releases"
        ...
        try:
            ...
            if h:
                try:
                    ...
                    if self.path_equal(file_path, self.file_BDFetch):
                        return False
...
####################################################################################################
    ...
    def init_engine_thread(self):
        try:
            ...
            if "-scan" in self.args_BDFetch:
                try:
                    idx = self.args_BDFetch.index("-scan")
                    self.trigger_context_scan(self.args_BDFetch[idx+1])
                except Exception:
                    pass
            
            with self.lock_config:
                first_launch = self.BDFetch_config.get("first_launch", True)
                driver_enabled = self.BDFetch_config.get("driver_switch", False)

            if not first_launch:
                with self.lock_config:
                    if self.BDFetch_config.get("process_switch"):
                        self.start_daemon_thread(self.protect_proc_thread)
                    if self.BDFetch_config.get("document_switch"):
                        self.start_daemon_thread(self.protect_file_thread)
                    if self.BDFetch_config.get("system_switch"):
                        self.start_daemon_thread(self.protect_system_thread)
                    if self.BDFetch_config.get("network_switch"):
                        self.start_daemon_thread(self.protect_net_thread)
    ...
    def scan_engine(self, file_path):
        with self.lock_config:
            ext_switch = self.BDFetch_config.get("extension_switch", False)
            sen_switch = self.BDFetch_config.get("sensitive_switch", False)
            ...
            for file_path in self.yield_files(targets):
                ...
                try:
                    ...
                    with self.lock_virus:
                        self.scan_count += 1
                    with self.lock_config:
                        suffix = self.BDFetch_config.get("suffix", [])
    ...
    def perform_cloud_scan(self, file_path):
        ...
        try:
            with self.lock_config:
                if not self.BDFetch_config.get("cloud_switch", True):
                    return False

                api_host, api_key, max_size = self.BDFetch_config.get("api_host"), self.BDFetch_config.get("api_key"), self.BDFetch_config.get("size", 100 * 1024 * 1024)
...
####################################################################################################
    ...
    def is_in_whitelist(self, file_path):
        ...
        with self.lock_config: 
            whitelist = self.BDFetch_config.get("white_list", [])
    ...
    def init_whitelist(self):
        self.manage_named_list("white_list", [self.file_BDFetch], action="add")
    ...
    def relock_file(self):
        while True:
            try:
                with self.lock_config:
                    quarantine_list = self.BDFetch_config.get("quarantine", [])
    ...
    def protect_proc_thread(self):
        ...
        while True:
            with self.lock_config:
                if not self.BDFetch_config.get("process_switch", False):
                    break
            try:
                time.sleep(0.1)
    ...
    def handle_new_process(self, pid, h=None):
        ...
        try:
            ...
            if "-scan" in cmdline and self.path_equal(process_file, self.file_BDFetch): return
    ...
    def protect_file_thread(self):
        ...
        try:
            ...
            while True:
                with self.lock_config:
                    if not self.BDFetch_config.get("document_switch", False): break
                try:
    ...
    def protect_net_thread(self):
        ...
        while True:
            with self.lock_config:
                if not self.BDFetch_config.get("network_switch", False): break
    ...
    def protect_system_thread(self):
        while True:
            with self.lock_config:
                if not self.BDFetch_config.get("system_switch", False):
                    break
            try:
                self.repair_system_mbr()
                self.repair_system_restrict()
    ...
    def capture_popup_window(self):
        try:
            ...
            while time.time() - start_time < 5:
                ...
                if pid.value != self.pid_BDFetch and pid.value > 4:
    ...
    def add_popup_rule(self, rule):
        ...
        with self.lock_config:
            target_list = self.BDFetch_config.setdefault("block_list", [])
    ...
    def popup_intercept_thread(self):
        ...
        while True:
            try:
                time.sleep(0.5)
                with self.lock_config:
                    rules = self.BDFetch_config.get("block_list", [])
                if not rules:
                    continue
    ...
    def install_system_driver(self):
        try:
            ...
            self._reg_write(winreg.HKEY_LOCAL_MACHINE, r"SYSTEM\CurrentControlSet\Services\BDFetch_Driver\Instances", "DefaultInstance", winreg.REG_SZ, "BDFetch Instance")
            self._reg_write(winreg.HKEY_LOCAL_MACHINE, r"SYSTEM\CurrentControlSet\Services\BDFetch_Driver\Instances\BDFetch Instance", "Altitude", winreg.REG_SZ, "320000")
            self._reg_write(winreg.HKEY_LOCAL_MACHINE, r"SYSTEM\CurrentControlSet\Services\BDFetch_Driver\Instances\BDFetch Instance", "Flags", winreg.REG_DWORD, 0)
    ...
    def stop_system_driver(self):
        try:
            ...
            subprocess.run(["sc", "stop", "BDFetch_Driver"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, shell=True)

            for _ in range(5):
                res = subprocess.run(["sc", "query", "BDFetch_Driver"], stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, shell=True)
                if "STOPPED" in res.stdout or "FAILED" in res.stderr or "1060" in res.stdout:
                    break

                time.sleep(0.1)

            subprocess.run(["sc", "delete", "BDFetch_Driver"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, shell=True)
            return True
        except Exception:
            return False
    ...
    def check_system_driver(self):
        try:
            return "RUNNING" in subprocess.run(["sc", "query", "BDFetch_Driver"], stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, shell=True).stdout
        except Exception:
            return False
    ...
    def pipe_server_thread(self):
        try:
            while True:
                with self.lock_config:
                    if not self.BDFetch_config.get("driver_switch", False):
                        break

                temp_port = ctypes.wintypes.HANDLE()
                if self.fltlib.FilterConnectCommunicationPort("\\BDFetch_Output_Pipe", 0, None, 0, None, ctypes.byref(temp_port)) == 0:
                    with self.lock_driver:
                    ...
                    while True:
                        ...
                        try:
                            if self.fltlib.FilterGetMessage(current_port, ctypes.byref(message), ctypes.sizeof(BDFetch_FULL_MESSAGE), None) == 0:
                                ...
                                with self.lock_config:
                                    block_codes = self.BDFetch_config.get("block", [])
...
if __name__ == "__main__":
    ...
    window = webview.create_window(
        title="BDFetch Security", url=f"http://127.0.0.1:{port_container[0]}/", width=init_width, height=init_height, x=pos_x, y=pos_y,
        frameless=True, easy_drag=False, js_api=js_api, background_color='#e0e0e0', hidden=hide_on_start)

    if platform.system() == "Windows":
        window_hook = WindowHook("BDFetch Security", js_api)
